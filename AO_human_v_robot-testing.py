...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\anhtn\\OneDrive - PennO365\\Documents\\GitHub\\AO_human_v_robot_main\\AO_human_v_robot.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    loopLoader = data.TrialHandler(nReps=1.0, method='sequential', 
        ...
        trialList=data.importConditions('stim_table_test.xlsx'),
    ...
    for thisLoopBlock in loopBlock:
        ...
        for thisLoopBreakCD in loopBreakCD:
            ...
            while continueRoutine and routineTimer.getTime() < 1.0:
                ...
                if skipBreak.status == STARTED and not waitOnFlip:
                    theseKeys = skipBreak.getKeys(keyList=['space'], ignoreKeys=["escape"], waitRelease=False)
        ...
        for thisLoopTrial in loopTrial:
            ...
            # keep track of which components have finished
            interTrialIntervalComponents = [textITI, skipITI, ITImarker]
            ...
            while continueRoutine:
                ...
                if ITImarker.status == NOT_STARTED and t >= 0.0-frameTolerance:
                    ...
                    cortex_obj.inject_marker(value=str('3000'), label='intertrial_interval', delta_time=delta_time)
            ...
            while continueRoutine:
                ...
                if fixationCrossMarker.status == NOT_STARTED and t >= 0.0-frameTolerance:
                    ...
                    fixationCrossMarker.status = STARTED
                    delta_time = tThisFlip-t  # Adding the extra time between now and the next screen flip
                    cortex_obj.inject_marker(value=str('1000'), label='fixationCross', delta_time=delta_time)
            ...
            while continueRoutine:
                ...
                if skipStim.status == STARTED and not waitOnFlip:
                    theseKeys = skipStim.getKeys(keyList=['space'], ignoreKeys=["escape"], waitRelease=False)
            ...
            if catchtrialQList[counterStim-1] == 'no':
                continueRoutine = False
            else:
